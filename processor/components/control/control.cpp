...
private:
    ...
    const std::map<unsigned, void(control::*)()> handler_for_opcode = {
        {opcode_t::HALT, &control::handle_HALT},
        {opcode_t::J, &control::handle_J},
        {opcode_t::JN, &control::handle_JN},
        {opcode_t::JZ, &control::handle_JZ},
        {opcode_t::LD, &control::handle_LD},
        {opcode_t::LRI, &control::handle_LRI},
        {opcode_t::ST, &control::handle_ST},
    };
...
{
    auto handler = handler_for_opcode.find(opcode);
    if (handler != handler_for_opcode.cend())
        (*this.*handler->second)();
    else
